...
import { GetAllBlog } from "@/actions/PostActions"
           ...
           <motion.article
            ...
            >
              <div className="aspect-[4/3] overflow-hidden rounded-lg border border-stone-200">
                <img
                  src={p.bannerImage || "/placeholder.svg"}
                  alt={`${p.title} banner image`}
                  className="h-full w-full object-cover"
                />
              </div>
              <div className="space-y-2 p-4">
                <h3 className="text-lg font-extrabold text-zinc-900">{p.title}</h3>
                <p className="text-sm leading-relaxed text-zinc-900/80">{p.description}</p>
